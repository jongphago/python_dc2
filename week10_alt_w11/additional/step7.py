...
def cancelOrder():
    print("기존 주문 내역: ", orderList)
    cancelMenu = input("취소할 메뉴를 입력하세요.: ")
    if cancelMenu in orderList.keys():
        cancelCount = int(input("취소할 수량을 입력하세요.: "))
        if orderList[cancelMenu] > cancelCount:
            orderList[cancelMenu] -= cancelCount
            print("해당 메뉴 %d개 취소되었습니다." % cancelCount)
        elif orderList[cancelMenu] == cancelCount:
            del orderList[cancelMenu]
            print("해당 메뉴 전체 취소되었습니다.")
        else:
            print("주문 수량보다 취소 수량이 많습니다.")
    else:
        print("주문 내역에 없는 메뉴입니다.")
...
while True:
    ...
    if orderType == 1:
        ...
        pay()
        cancel = input("주문 내역 수정: y / 주문 최종 완료: n: ")
        if cancel == "y":
            cancelOrder()
            print("주문 내역과 총 금액을 확인해 주세요.")
            pay()
